...
func effectiveDCAEntryPrice(entry core.DCAEntry) float64 {
...
func decorateDCAEntries(entries []core.DCAEntry) []core.DCAEntry {
	if len(entries) == 0 {
		return nil
	}

	result := append([]core.DCAEntry(nil), entries...)
...
func buildDCASummary(item core.WatchlistItem) *core.DCASummary {
	validEntries := make([]core.DCAEntry, 0, len(item.DCAEntries))
...
func buildPositionSummary(item core.WatchlistItem) *core.PositionSummary {
	summary := &core.PositionSummary{
...
func decorateItemDerived(item core.WatchlistItem) core.WatchlistItem {
...
func buildMarketSnapshot(item core.WatchlistItem, series core.HistorySeries) *core.MarketSnapshot {
