...
- Window defaults come from the desktop shell, but `UseNativeTitleBar` is read from the store snapshot and fed into `internal/platform.BuildMainWindowOptions(...)`, which changes title-bar behavior on macOS, Windows, and Linux.
- `BuildMainWindowOptions` sets `1200x828`, `MinWidth=1200`, `MinHeight=828`, a desktop background colour, Windows theme inheritance, and macOS translucent backdrop defaults.
- When `UseNativeTitleBar=false`, macOS uses `MacTitleBarHiddenInsetUnified` so the native traffic-light controls remain available; Windows and Linux use Wails `Frameless` and depend on frontend custom controls.
- macOS double-click maximize/restore for custom chrome is implemented in `AppHeader.vue` via `frontend/src/wails-runtime.ts`, because hidden visible title bars do not automatically provide the native double-click behavior.
...
- `frontend/src/wails-runtime.ts` centralises desktop window runtime calls and must no-op safely when the frontend runs in the Vite browser dev server.
...
- Keep Windows and Linux frameless support in mind when touching shell layout, key bindings, build flags, or OS-specific commands.
