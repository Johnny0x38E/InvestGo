...
5. Handle window and platform options carefully. Changes to title bar, background, DevTools, or key bindings must continue to respect settings-driven behavior and the helper boundaries in `internal/platform/window.go` and `internal/platform/proxy.go`.
6. Remember the frontend runs inside a desktop webview, not a normal browser tab. Avoid adding browser assumptions that fight the desktop shell, local routing model, or future Windows support.
