...
- 后端：Go 1.26、Wails v3 alpha.74、标准 HTTP handler。
...
- Windows 构建：PowerShell 脚本以及 `go`、`npm`、Microsoft Edge WebView2 Runtime，本地图标渲染需要 ImageMagick。
...
- Windows 本地生成 `build/appicon.png` 时需要 ImageMagick `magick`
...
Windows 构建脚本会在缺少 `build/appicon.png` 时用 ImageMagick 生成图标，执行 `npm run build`，并输出 `build/bin/investgo-windows-amd64.exe`。
