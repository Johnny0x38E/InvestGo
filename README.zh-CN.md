...
InvestGo 是一个基于 Wails 的桌面投资跟踪工具，用于管理自选、持仓、组合分析、热门榜单、历史走势图、开发日志和价格提醒。

InvestGo 主要把 Wails 用作 Go 后端和 Vue 前端的轻量桌面容器。应用数据通过本地 HTTP `/api/*` 接口提供，因此前端仍然可以在普通 Vite 中独立运行，而打包后的桌面应用不需要像 Electron 一样携带 Chromium 和 Node.js 运行时。对于本项目这种形态，Wails 通常可以带来更小的应用体积、更低的空闲内存占用和更快的启动速度，同时仍然提供原生窗口、嵌入资源、生命周期钩子、DevTools 支持和平台集成能力。
...
本仓库不是 monorepo。Go module 根目录就是仓库根目录，前端位于 `frontend/`。
