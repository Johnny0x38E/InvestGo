...
const marketSnapshot = computed(() => {
    ...
    const hasLiveAndSeries =
        item.currentPrice > 0 && series != null && series.startPrice > 0;
    const effectiveChange = hasLiveAndSeries
        ? item.currentPrice - series.startPrice
        : (series?.change ?? item.change ?? 0);
    const effectiveChangePct = hasLiveAndSeries
        ? ((item.currentPrice - series!.startPrice) / series!.startPrice) * 100
        : (series?.changePercent ?? item.changePercent ?? 0);
    ...
    const amplitudePct =
        previousClose > 0 && rangeHigh > 0 && rangeLow > 0
            ? ((rangeHigh - rangeLow) / previousClose) * 100
            : 0;
    ...
    const positionBaseline =
        item.quantity > 0 ? item.quantity * item.costPrice : 0;
    const positionPnL =
        item.quantity > 0 ? positionValue - positionBaseline : 0;
    const positionPnLPct =
        positionBaseline > 0 ? (positionPnL / positionBaseline) * 100 : 0;
    const positionTone: MarketMetricCard["tone"] =
        positionValue > positionBaseline
            ? "rise"
            : positionValue < positionBaseline
              ? "fall"
              : "neutral";
...
const marketOverview = computed(() => {
    ...
    return {
        ...
        changeLabel: t("market.changeLabel", {
...
const positionDetail = computed(() => {
    ...
    return {
        hasPosition,
        value: hasPosition
            ? formatUnitPrice(snapshot.positionValue, snapshot.item.currency)
            : "-",
        pnl: hasPosition ? formatMoney(snapshot.positionPnL, true) : "-",
        pnlPct: hasPosition ? formatPercent(snapshot.positionPnLPct) : "-",
        costBasis: hasPosition
            ? formatUnitPrice(snapshot.positionBaseline, snapshot.item.currency)
            : "-",
        costPrice: hasPosition
            ? formatUnitPrice(snapshot.item.costPrice, snapshot.item.currency)
            : "-",
        ...
        },
        {
            ...
            sub:
                snapshot.previousClose > 0
                    ? t("market.cards.amplitudeEstimated")
                    : t("market.cards.amplitudePending"),
            tone: "neutral",
        },
    ];
});
        ...
        <div class="market-board">
            ...
            <aside class="market-aside">
                <div v-if="marketOverview" class="market-inspector">
                    <section
                        class="market-hero"
                        :class="`tone-${marketOverview.tone}`"
                    >
                        <div class="market-hero-topbar">
                            <div class="market-hero-intervals">
                                <button
                                    ...
                                    type="button"
                                    @click="
                                        $emit('select-interval', entry.value)
                                    "
                                >
                                    {{ entry.label }}
                                </button>
                        ...
                        <div class="market-hero-main">
                            <strong class="market-hero-price">{{
                                marketOverview.price
                            }}</strong>
                            <div class="market-hero-delta">
                                <span class="market-hero-delta-label">{{
                                    marketOverview.changeLabel
                                }}</span>
                                <b class="market-hero-delta-val">{{
                                    marketOverview.changeValue
                                }}</b>
                                <span class="market-hero-delta-pct">{{
                                    marketOverview.changePercent
                                }}</span>
                            </div>
                        </div>
                        ...
                        <footer class="market-hero-foot">
                            ...
                            <span class="market-hero-badge">{{
                                t("market.meta.chartSource", {
                                    source: marketOverview.chartSource,
                                })
                            }}</span>
                            <span class="market-hero-badge">{{
                                t("market.meta.syncedAt", {
                    ...
                    <div class="market-metrics">
                        <article
                            ...
                            :class="
                                positionDetail.hasPosition
                                    ? `tone-${positionDetail.tone}`
                                    : ''
                            "
                        >
                            <span class="market-pos-label">{{
                                t("market.position.title")
                            }}</span>
                            <template v-if="positionDetail.hasPosition">
                                <div class="market-pos-main">
                                    <div class="market-pos-stat">
                                        <strong class="market-pos-value">{{
                                            positionDetail.value
                                        }}</strong>
                                        <span class="market-pos-stat-label">{{
                                            t("market.position.currentValue")
                                        }}</span>
                                    </div>
                                    <div
                                        class="market-pos-stat market-pos-stat--right"
                                    >
                                        <b class="market-pos-pnl">{{
                                            positionDetail.pnl
                                        }}</b>
                                        <span class="market-pos-pnl-pct">{{
                                            positionDetail.pnlPct
                                        }}</span>
                                    </div>
                                </div>
                                <div class="market-pos-detail">
                                    <div class="market-pos-stat">
                                        <span class="market-pos-stat-label">{{
                                            t("market.position.costPrice")
                                        }}</span>
                                        <span class="market-pos-detail-val">{{
                                            positionDetail.costPrice
                                        }}</span>
                                    </div>
                                    <div
                                        class="market-pos-stat market-pos-stat--right"
                                    >
                                        <span class="market-pos-stat-label">{{
                                            t("market.position.quantity")
                                        }}</span>
                                        <span class="market-pos-detail-val">{{
                                            t("market.position.quantityValue", {
                            ...
                            <span v-else class="market-pos-empty">{{
                                t("market.position.empty")
                            }}</span>
                        </article>
                        ...
                        >
                            ...
                            <strong class="metric-strip-value">{{
                                card.value
                            }}</strong>
